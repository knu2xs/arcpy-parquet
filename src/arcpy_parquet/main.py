...
def feature_class_to_parquet(
    ...
    assert geometry_format in ('XY', 'WKT', 'WKB'), 'Geometry output format must be either XY, WKB or WKT.'
...
def parquet_to_feature_class(
    ...
    parquet_path = parquet_path if isinstance(parquet_path, Path) else Path(parquet_path)
    output_feature_class = output_feature_class if isinstance(output_feature_class, Path) else Path(
    ...
    if parquet_path.is_dir():
        ...
        assert len(pqt_prts) > 0, ('The provided directory and partitions do not appear to contain any parquet '
                                   'part files.')
    elif parquet_path.is_file():

        assert parquet_partitions is None, "If providing a parquet part file, you cannot specify a parquet partition."
        assert parquet_path.stem.startswith('part-'), 'The provided file does not appear to be a parquet part file.'

        # assemble into list so iteration below works
        pqt_prts = [parquet_path]

    else:
        raise ValueError('parquet_path must be either a directory for parquet data or a specific part file.')

    assert geometry_type in geom_dict.keys(), (f'geometry_type must be from the following list '
                                               f'[{", ".join(geom_dict.keys())}]')
    ...
    if schema_file is None:
        ...
        # dump to a dict
        schema_dict = {k: v for k, v in zip(schema_df['field_name'], schema_df.to_dict('records'))}
...
def create_schema_file(template_feature_class_path: Path, output_schema_file: Path) -> Path:
    ...
    desc_keys = ['name', 'type', 'precision', 'scale', 'length', 'aliasName', 'isNullable', 'required', 'domain']
    param_keys = ['field_name', 'field_type', 'field_precision', 'field_scale', 'field_length', 'field_alias',
